...
obs <- read.table('lake_wide_csv/temp_out_mq3.txt')
frac <- read.table('lake_wide_csv/frac_out_mq3.txt')
...
#glo_dly <- read.table('csv/bi2m_T3D.csv', row.names=1, sep=',')
...
seldts <- grep('2019-..-.. 12:00:00', dts_mod) # select the 12Z timestep to compare with obs
...
# generate table
deps <- read.table('lake_wide_csv/depths_by_lake.txt')[,-3]
#dep_diff <- -apply(deps, 1, diff)/deps[,'ctl']
#dep_diff <- -apply(deps, 1, diff)/deps[,'ctl']

tbl <- data.frame(name=row.names(meta), area_km=meta$area_km)
